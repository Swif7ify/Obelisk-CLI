...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider('obeliskSummary', summaryView)
    );
    ...
    context.subscriptions.push(
        treeView.onDidChangeSelection((e) => {
            ...
            if (item instanceof FindingItem && item.resourceUri && item.line !== undefined) {
...
async function runScan(): Promise<void> {
    ...
    try {
        ...
        diagnosticsManager.updateDiagnostics(result, workspaceFolder.uri.fsPath);
