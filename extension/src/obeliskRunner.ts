		...
		return new Promise<ObeliskScanResult>((resolve, reject) => {
			...
			this.process.on("close", (code) => {
				...
				try {
					...
					if (jsonStart === -1) {
						reject(
							new Error(
								"No JSON output from Obelisk. Is it installed and on PATH?",
							),
						);
						return;
					}
					...
					reject(
						new Error(
							`Failed to parse Obelisk output: ${e.message}`,
						),
					);
				}
			});
			...
			this.process.on("error", (err) => {
				...
				if ((err as any).code === "ENOENT") {
					reject(
						new Error(
							`Could not find '${executable}'. Install Obelisk CLI (https://github.com/Swif7ify/Obelisk-CLI) or set obelisk.executablePath in settings.`,
						),
					);
				} else {
					reject(err);
