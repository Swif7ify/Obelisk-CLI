...
// FormatMarkdown generates a markdown report from scan results and AI analysis.
func FormatMarkdown(result *scanner.ScanResult, report *ai.HealthReport) string {
	var sb strings.Builder

	// Header with timestamp
	sb.WriteString("# 🏛️ Obelisk Health Report\n\n")
	sb.WriteString(fmt.Sprintf("**Generated:** %s\n", time.Now().Format("2006-01-02 15:04:05")))
	sb.WriteString(fmt.Sprintf("**Project Path:** `%s`\n", result.ProjectPath))
	sb.WriteString(fmt.Sprintf("**Project Type:** %s\n\n", result.ProjectType))
	...
	if report != nil {
		...
		sb.WriteString("|---|---|\n")
		sb.WriteString(fmt.Sprintf("| Overall | %d/100 |\n", report.OverallScore))
		sb.WriteString(fmt.Sprintf("| Security | %d/100 |\n", report.SecurityScore))
		sb.WriteString(fmt.Sprintf("| Architecture | %d/100 |\n", report.ArchitectureScore))
		sb.WriteString(fmt.Sprintf("| Quality | %d/100 |\n\n", report.QualityScore))
	}
	...
	sb.WriteString("---\n")
	sb.WriteString("*Generated by Obelisk CLI*\n")
